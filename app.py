...
class QuiLeAutovocal(tk.Tk):
    ...
    def _ui(self):
        ...
        b=ttk.Frame(r); b.pack(fill="x",pady=(8,0)); self.btn=ttk.Button(b,text="Bắt đầu monitor",command=self._toggle); self.btn.pack(side="left"); ttk.Button(b,text="Ghi âm 10 giây",command=self._record).pack(side="left",padx=8); self.status=ttk.Label(b,text="Sẵn sàng"); self.status.pack(side="right"); self._load()
